...
export default function MomentPeakCard({ peaks }) {
  return (
    <motion.div 
      ...
      className="glass-panel p-6 rounded-2xl flex-1 border-t-4 border-secondary"
    >
      ...
      <div className="space-y-3">
        {peaks.map((peak, idx) => (
          ...
          >
            <div className={`w-10 h-10 rounded flex items-center justify-center ${peak.type === 'euphoria' ? 'bg-primary/20 text-primary' : 'bg-error/20 text-error'}`}>
            ...
            <span className={`font-bold ${peak.type === 'euphoria' ? 'text-primary' : 'text-error'}`}>{peak.value}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
